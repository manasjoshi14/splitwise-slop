...
export default function Friends() {
  ...
  useEffect(() => {
    Promise.all([api.get('/api/friends'), api.get('/api/balances')])
    ...
    <div className="p-4 pb-20">
      ...
      {friends.length === 0 ? (
        <p className="text-gray-500 text-center py-8">
          No friends yet. Add someone to a group or create a 1-on-1 expense!
        </p>
      ) : (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <img
                      src={
                        f.avatar_url ||
                        `https://ui-avatars.com/api/?name=${encodeURIComponent(f.name)}&background=random`
                      }
                      alt={f.name}
                  ...
                  {balance !== 0 && (
                    <p
                      className={`text-sm font-medium ${balance > 0 ? 'text-green-600' : 'text-orange-600'}`}
                    >
                      {balance > 0
