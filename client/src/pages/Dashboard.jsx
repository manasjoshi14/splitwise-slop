...
export default function Dashboard() {
  ...
  const totalOwed = balances.filter(b => b.balance > 0).reduce((s, b) => s + parseFloat(b.balance), 0);
  const totalOwe = balances.filter(b => b.balance < 0).reduce((s, b) => s + Math.abs(parseFloat(b.balance)), 0);
    ...
    <div className="p-4 pb-20">
      ...
      <div className="grid grid-cols-2 gap-3 my-4">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm">
          ...
          <p className="text-2xl font-bold text-orange-600">${totalOwe.toFixed(2)}</p>
        </div>
      </div>
      ...
      {balances.length === 0 ? (
        <p className="text-gray-500 text-center py-8">No balances yet. Add an expense to get started!</p>
      ) : (
