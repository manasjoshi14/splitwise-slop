      ...
      {friendBalance && (
        <div className="bg-white rounded-xl p-4 shadow-sm mb-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img
              src={friendBalance.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(friendBalance.name)}&background=random`}
              alt={friendBalance.name}
              className="w-12 h-12 rounded-full"
            />
            <div>
              <h1 className="text-xl font-bold">{friendBalance.name}</h1>
              <p className={`text-sm ${parseFloat(friendBalance.balance) > 0 ? 'text-green-600' : 'text-orange-600'}`}>
